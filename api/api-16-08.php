...
else
{
    ...
    switch ($ServerRequest)
	{
        ...
        case "product":  //  Category based Product
        Product($conn);
        break;
        ...
        case "search":  //  All Product Search
        SearchProduct($conn);
        break;	
		...
		case "add_wishlist":  // Add Product Wish List
        AddWishlist($conn);
        break;
		
		case "wish_list":  // Listing Wishlist
		WishListing($conn);
		break;
		...
		case "view_addrees": // View Customer
		AddressView($conn);
		break;
		
		case "quantity": // Change Quantity
		ChangeQuantity($conn);
		break;
		
		case "search":  // Serach Product
        Search($conn);
        break;
			...
			if($result->num_rows>0){
				 ...
				 if($row->status=='0'){
				    ...
				    echo json_encode($data);
					 ...
					 $carts = $conn->query("SELECT * FROM cart WHERE customer_id='".$row->id."'");
					 $array_post['cart_num'] = $carts->num_rows;
					 
					 $wish = $conn->query("SELECT * FROM wishlist WHERE customer_id='".$row->id."' AND status='1'");
					 $array_post['wish_num'] = $wish->num_rows;
	...
	function productDetail($conn){
		...
		if($product_id!=''){
			...
			$array_data['product_image'] = $productDetails->product_image.','."demo.jpg";
			...
			$array_data['product_specification'] = "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor.";
			$array_data['product_shiping'] = "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Donec quam felis,";
			$array_data['product_return'] = "Lorem ipsum dolor sit amet, consectetuer adipiscing elit. Aenean commodo ligula eget dolor. Aenean massa. Cum sociis natoque penatibus et magnis dis parturient montes, ";
		...
		if($customer_id!="" && $product_id!=""){
			...
			if($checkExit->num_rows > 0){
		      ...
		      echo json_encode($data);
				...
				$insertData = $conn->query("INSERT INTO cart SET customer_id='$customer_id',product_id='$product_id',color='$color',size='$size',status='1',created_on=Now()");
		    ...
		    if($Listing->num_rows>0){
			  ...
			  while($cart_data =$Listing->fetch_object()){
			   ...
			   $product_result = $product->fetch_object();
				...
				$array_data['product_color'] = $cart_data->color;
				$array_data['product_size'] = $cart_data->size;
	...
	function ChangeQuantity($conn){ 
	  ...
	  if($cart_id!='' && $quantity!=''){
		     ...
		     $update = $conn->query("UPDATE cart SET quantity='$quantity' WHERE id='".$cart_id."'");
